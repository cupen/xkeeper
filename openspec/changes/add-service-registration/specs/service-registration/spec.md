...
- `ExecStart` 为当前 xkeeper 可执行文件的绝对路径，后接 `run -c <核心配置文件绝对路径>`（核心配置路径按 `-c` 解析后的路径；未指定 `-c` 时用平台默认路径）。
...
- **THEN** `/etc/systemd/system/xkeeper.service` 存在且 ExecStart 指向当前可执行文件与解析后的核心配置路径，`systemctl is-enabled xkeeper` 返回 enabled
